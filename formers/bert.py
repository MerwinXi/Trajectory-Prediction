...
from .utils import PositionedFeedForward, SublayerConnection
...
class MultiHeadedAttention(nn.Module):
    ...
    def __init__(self, head, model_dimension, dropout):
        ...
        self.linear_layers = nn.ModuleList(
            [nn.Linear(model_dimension, model_dimension) for _ in range(3)]
        )
...
class BERT(nn.Module):
    def __init__(self, args):
        ...
        self.transformer_blocks = nn.ModuleList(
            [TransformerBlock(args) for _ in range(self.bert_num_blocks)]
        )
